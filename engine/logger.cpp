...
void Logger::logDebug(const std::string &tag, const char *format, ...) const {
  if (logLevel <= LOG_LEVEL_DEBUG) {
    va_list ap;
    va_start(ap, format);
    fprintf(stdout, "[%sDEBUG%s][%s]: ", ANSI_COLOR_MAGENTA, ANSI_COLOR_RESET,
            tag.c_str());
...
void Logger::logInfo(const std::string &tag, const char *format, ...) const {
  if (logLevel <= LOG_LEVEL_INFO) {
    va_list ap;
    va_start(ap, format);
    fprintf(stdout, "[%sINFO%s][%s]: ", ANSI_COLOR_CYAN, ANSI_COLOR_RESET,
            tag.c_str());
    vfprintf(stdout, format, ap);
    fprintf(stdout, "\n");
    va_end(ap);
...
void Logger::logWarning(const std::string &tag, const char *format, ...) const {
  if (logLevel <= LOG_LEVEL_WARNING) {
    va_list ap;
    va_start(ap, format);
    fprintf(stdout, "[%sWARNING%s][%s]: ", ANSI_COLOR_YELLOW, ANSI_COLOR_RESET,
            tag.c_str());
    vfprintf(stdout, format, ap);
    fprintf(stdout, "\n");
    va_end(ap);
